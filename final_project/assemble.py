...
def instr_to_bin(instr, line_num):
    ...
    if instr_type == "R":
        ...
        imm = bin(
            (labels[instr[3]] - line_num - 1 if bne_or_blt else int(instr[3])) & 0x1FFFF
        )[2:].zfill(17)
...
def valid_operands(mnemonic, operands):
    ...
    if instr_type == "R":
        return (
            (operands[0] in aliases)
            and (operands[1] in aliases)
            and (
                (is_int(operands[2]) and (0 <= int(operands[2]) < 32))
                if sll_or_sra
                else (operands[2] in aliases)
            )
        )
    elif instr_type == "I":
        return (
            ...
            and (
                True
                if bne_or_blt
                else (is_int(operands[2]) and (-(2**16) <= int(operands[2]) < 2**16))
            )
        )
    elif instr_type == "JI":
        if mnemonic == "setx":
...
parser.add_argument(
    "-i", "--instr_file", help="csv file containing custom instruction definitions"
)
parser.add_argument(
    "-r", "--alias_file", help="csv file containing custom register aliases"
)
...
if instr_file and not instr_file.endswith(".csv"):
    invalid_argv_exp += (
        "assemble.py: error: custom instruction file must end with .csv\n"
    )
    invalid_argv = True
if invalid_argv:
    ...
    with open(instr_file, newline="") as f:
        ...
        for i, instr in enumerate(instructions):
            ...
            if opcodeR in instrs_inv:
                ...
                if (instr_type == "R" and ref_instr_type == "R") or (
                    instr_type != "R" and ref_instr_type != "R"
                ):
                    opcode_is_dupe = True
                    conflict_instrs[opcodeR].append(
                        f"Line {i + 1}: {','.join(instr)}\n"
                    )
...
with open(input_file, "r", encoding="utf8") as f:
    for i, line in enumerate(f):
        ...
        if (
            mnemonic == "bne"
            or mnemonic == "blt"
            or instrs[mnemonic][0] == "JI"
            and not operands[-1].isdigit()
        ):
